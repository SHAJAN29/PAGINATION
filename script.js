  ...
  let gettingFor = document.querySelector(".btn");
...
function fun(e){
   ...
   let show_data = document.querySelector('#show')

    console.log(e);

   let evalue = parseInt(e.getAttribute("for"))

let beta =data[evalue-1]

let {name,id,email}=beta;

console.log(beta);

 show_data.innerHTML = `
 
 <img src="https://img.icons8.com/bubbles/512/user-male.png" alt="" srcset="">
 <div class="empty">
<p for ="${id}" class="idss">ID: ${id} </p>
<p>NAME :  ${name}</p>
<p>EMAIL : ${email} </p>
...
function next(e){
    let userData =100;
// let iter_value =(parseInt(userData/5))*5;
// let check =userData%5;
// console.log(check);
// console.log(iter_value);




for (let i = 1; i <= 5; i++) {
    let gettingFor = document.querySelector(`.btn-${i}`);

    let forAttribute = +(gettingFor.getAttribute('for'));
 
    let adding_forAttribute_value =forAttribute +5;
 if(adding_forAttribute_value <= userData){
   
    
  
       
 gettingFor.setAttribute('for',adding_forAttribute_value);
 gettingFor.innerText =adding_forAttribute_value;
 console.log( gettingFor.innerText =adding_forAttribute_value);


 }
//  for (let i = adding_forAttribute_value; i >= userData; i++) {
//     let removable = document.querySelector(`[for="${i}"]`)
//     removable.remove()
//     break;
//   } 
